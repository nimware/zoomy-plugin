...
a).load(function(){var d=f.glare?"<span/>":"",e=l.border(b);a=a.replace(" ","%20");g[c].zoom={x:b.find("img").width(),y:b.find("img").height(),border:e[1]};b.append(d).css({"background-image":"url("+a+")"}).find("img").remove();l.glare(b)}).each(function(){(this.complete||e.browser.msie&&parseInt(e.browser.version,10)===6)&&e(this).trigger("load")})},zoom:function(a,b){g[b]={state:null,index:b};g.count.push(0);var c=typeof a.attr(f.attr)==="string"&&f.attr!=="href"?a.attr(f.attr):a.attr("href"),d=
null,r=f.zoomInit;(function(){var c=[],e=function(b){i.move(a,d,b)},l=function(){i.enter(a,d);a.bind("mousemove",e);i.callback(f.zoomStart,d)},m=function(b){i.leave(a,d,b);a.unbind("mousemove",e);i.callback(f.zoomStop,d)},h=function(c){g.pos=c;f.clickable||c.preventDefault();g[b].state===0||g[b].state===null?(l(),i.move(a,d,c)):g[b].state===1&&j!=="mouseover"&&j!=="mouseenter"&&m(0)},r=function(a){g.pos=a;g[b].state===0&&l()},o=function(){return false};j==="mouseover"?c[j]=h:(c[j]=h,c.mouseover=r);
if(!f.clickable&&j!=="click")c.click=o;c.mouseleave=function(a){g.pos=a;g[b].state===1&&m(null)};a.bind(c)})();a.addClass("parent-zoom").append('<div class="zoomy zoom-obj-'+b+'" rel="'+b+'"><img id="tmp"/></div>');d=e(".zoom-obj-"+b);r!==null&&typeof r==="function"&&r(a);l.params(a,d);m.image(c,d)},init:function(a,b){b.one("load",function(){m.zoom(a,g.count.length)}).each(function(){(this.complete||e.browser.msie&&parseInt(e.browser.version,10)===6)&&e(this).trigger("load")})}};typeof j==="object"&&
